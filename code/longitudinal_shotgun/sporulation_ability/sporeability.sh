...
# Species that were found by MetaPhlAn4, but we havent determined their sporulation ability yet. (They were not a part of MIDAS analysis) 
# ~/projecst/thesis/species_to_define_GCF.csv created with ~/projecst/thesis/longitudinal_shotugn/species_define_spore_metaphlan.R


## Download genomes 
# Input file with GTDB accessions in the first column
 genomes_dir="/home/nlzoh.si/ursmik1/projects/spore_evo_dynamics/data/sporulation_signature/all_gtdb_genomes/additional_mpa_genomes"

mkdir -p "$genomes_dir"

# Read accession numbers from the first column 
cut -f2 ~/projects/thesis/species_to_define_GCF.tsv | while read -r accession; do
    # Clean up whitespace
...
genomes_dir="/home/nlzoh.si/ursmik1/projects/spore_evo_dynamics/data/sporulation_signature/all_gtdb_genomes/additional_mpa_genomes/extracted_fna/"
output="/home/nlzoh.si/ursmik1/projects/spore_evo_dynamics/data/sporulation_signature"

mkdir -p "${output}/blast_tmp"

# Output file with header
output_file="${output}/additional_mpa_blast_results.tsv"
